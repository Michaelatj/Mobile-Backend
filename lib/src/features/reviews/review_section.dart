...
class _ReviewSectionState extends ConsumerState<ReviewSection> {
  ...
  Future<void> _syncRating() async {
    final reviews = await ReviewDao.getReviewsByProvider(widget.providerId);
    final avg = reviews.isEmpty
        ? 0.0
        : reviews.map((r) => r.stars).reduce((a, b) => a + b) / reviews.length;
    // simpan dengan 1 desimal agar konsisten di UI
    final rounded = double.parse(avg.toStringAsFixed(1));
    await ProviderDao.updateProviderRating(widget.providerId, rounded);
    // invalidate providers so consumers reload (Home, detail)
